...
def get_random_user_agent():
    ...
    edge_versions = [
        ...
        for major in range(100, 125)
        for minor in range(0, 5)
        for patch in range(1000, 1100)
        for build in range(50, 70)
    ]
    ...
    if browser_choice == "Chrome":
        ...
        # Exemplo: Chrome/109.0.0.0 + Edg/109.0.1518.78
        chrome_major = version.split('.')[0]  # ex: '109'
        return (
...
def save_singlefile(link: Link, out_dir: Optional[Path] = None, timeout: int = TIMEOUT) -> ArchiveResult:
    """
    Download full site usando SingleFile, 
    adicionando rotação de User-Agent e proxy para Chrome.
    """
    out_dir = out_dir or Path(link.link_dir)
    output = "singlefile.html"

    # Gera o user-agent aleatório (para esse "job")
    random_ua = get_random_user_agent()

    # Obtém os args padrão do Chrome (exceto o primeiro),
    # e remove qualquer "--user-agent=..."
    ...
    # Formato do proxy para Chrome: --proxy-server=http://usuario:senha@host:port
    filtered_browser_args.append(f"--user-agent={random_ua}")
    filtered_browser_args.append(f"--proxy-server={ZENROWS_PROXY}")  

    # Constrói a string JSON que SingleFile usará internamente para abrir o Chrome
    browser_args = '--browser-args={}'.format(json.dumps(filtered_browser_args))

    # Constrói as opções a partir de SINGLEFILE_ARGS,
    # adiciona o caminho do Chrome e o browser_args
    options = [
        *SINGLEFILE_ARGS,
        f"--browser-executable-path={CHROME_BINARY}",
        browser_args,
    ]
    # Remove qualquer user-agent duplicado que eventualmente exista em SINGLEFILE_ARGS
    options = [opt for opt in options if not opt.startswith("--user-agent")]

    # Define parâmetros extras para singlefile (JS):
    additional_params = [
        ...
        "--compress-HTML=true",
        # Passamos também o user-agent para SingleFile (caso ele internamente faça outra verificação)
        f"--user-agent={random_ua}",
    ]

    # Evitar duplicar flags se existirem em SINGLEFILE_ARGS
    seen_option_names = []
    ...
    print("Comando SingleFile =>", cmd)

    status = 'succeeded'
    timer = TimedProgress(timeout, prefix='      ')
    try:
        # Executa o SingleFile
        ...
        status = 'failed'
        # Ajusta a string se houver aspas
        output = err
    finally:
        timer.end()
